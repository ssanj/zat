  ...
  fn config_fails_to_load_if_template_dir_does_not_exist() {
    ...
    match get_user_config_fallable(user_config_provider, args) {
      ...
      Err(error) => {
        ...
        assert_eq!(error, ZatError::ProcessCommandError(ProcessCommandErrorReason::UserConfigError(UserConfigErrorReason::TemplateDirDoesNotExist(expected_error, expected_fix))))
      }
    }
  }
